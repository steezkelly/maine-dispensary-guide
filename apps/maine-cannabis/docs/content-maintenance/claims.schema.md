...
Required fields per claim: `claim_id`, `canonical_path`, `claim_class`, `claim_summary`, `source_id`, `source_url`, `source_class`, `source_publication_or_effective_date`, `mdg_verification_date`, `owner`, `review_cadence_days`, `dependent_pages`, and `propagation_sla_days`. Each `dependent_pages` entry is an object with `canonical_path` and `completed_at` (RFC3339 UTC or `null` while pending). `correction_log_reference` is required when a correction exists and otherwise must be `null`.
