...
**Data:** `apps/maine-cannabis/data/ga4-pull-2026-07-11/` (JSONL per query + index.md + dashboard.html)
**This file is intentionally outside that dir** so re-running the pull doesn't wipe the analysis.
...
## Known visualization issues (deferred)

The dashboard has one cosmetic issue: the device-category doughnut chart overflows its chart-box bounds at full-page screenshot. Fix: add `max-height: 500px` to `.chart-box` CSS. Not blocking; the chart is fully visible when scrolling.