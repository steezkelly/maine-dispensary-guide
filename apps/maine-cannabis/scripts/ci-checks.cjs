...
function checkFile(filePath) {
  ...
  // Check for hardcoded site URLs (except in legitimate content)
  const urlMatches = content.match(HARDCODED_URL_PATTERN);
  if (urlMatches) {
    urlMatches.forEach(url => {
      // Skip external CDN URLs that are always hardcoded
      if (url.includes('googleapis.com') || url.includes('gstatic.com') ||
          url.includes('googletagmanager.com') || url.includes('transparenttextures.com')) return;
      issues.push(`Hardcoded URL: ${url}`);
    });
  }
