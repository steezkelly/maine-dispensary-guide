...
function makeInvestigationPacket(row) {
  ...
  return {
    ...
    created_at: row.detected_at || new Date().toISOString(), updated_at: row.detected_at || new Date().toISOString(),
...
function buildOpportunityEngine(input) {
  ...
  for (const row of rows) {
    ...
    eventLedger.push({ event: 'EVIDENCE_REFRESH', opportunity_id: cases.get(key).opportunity_id, deduplication_key: key, window_start: row.window_start || row.date || null, window_end: row.window_end || row.date || null, state: row.state, appended_at: row.detected_at || new Date().toISOString() });
  }
...
function applyDiagnosticUpdate(packet, result) {
  ...
  const ledgerEntry = { diagnostic_id: result.diagnostic_id, executed_at: result.executed_at || new Date().toISOString(), input_provenance: result.input_provenance || null, output_artifact: result.output_artifact || null, result_summary: result.result_summary || null, evidence_quality: result.evidence_quality || 'partial', hypothesis_updates: { supporting: [...supporting], contradicting: [...contradicting] }, production_touch: false };
