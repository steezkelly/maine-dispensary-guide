...
const rawJoinPageWindow = join.joinPageWindow;
const TEST_AS_OF = '2026-07-16';
function joinPageWindow(input) { return rawJoinPageWindow({ ...input, asOf: input.asOf || TEST_AS_OF }); }
...
test('GA4-only rows are explicit', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/x' }, data_api_value: 3 }] }] }, vercelRows: [] });
...
test('GA4-only rows are blocked until the required Vercel counterpart is available', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/x' }, data_api_value: 3 }] }] }, vercelRows: [], asOf: '2026-07-16' });
...
test('Vercel-only rows are explicit', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [] }, vercelRows: [{ day: '2026-07-12', requestPath: '/x', value: 4 }] });
...
test('Vercel-only observations retain Vercel provenance rather than a GA4 canonical source', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [] }, vercelRows: [{ day: '2026-07-12', requestPath: '/x', value: 4, source_family: 'pageviews' }] });
  assert.equal(rows[0].canonical_metric_source, 'vercel_a4');
});
test('source delta preserves both values', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/x' }, data_api_value: 8 }] }] }, vercelRows: [{ day: '2026-07-12', requestPath: '/x', value: 6 }] });
...
test('page manifest identity is attached', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/x' }, data_api_value: 1 }] }] }, vercelRows: [], manifestRows: [{ page_id: 'page-x', canonical_path: '/x' }] });
  assert.equal(rows[0].page_manifest_row_key, 'page-x');
});
test('row key joins query variants to same page', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: 'https://example.com/x/?utm=1' }, data_api_value: 1 }] }] }, vercelRows: [{ day: '2026-07-12', requestPath: '/x/', value: 1 }] });
  assert.equal(rows.length, 1); assert.equal(rows[0].source_presence, 'both');
});
test('different dates do not cross-join', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260711', page_path: '/x' }, data_api_value: 1 }] }] }, vercelRows: [{ day: '2026-07-12', requestPath: '/x', value: 1 }] });
  assert.equal(rows.length, 2);
});
test('pre-CSP conversion window is degraded', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R3_event_count_daily', sanitized_rows: [{ row_key: { date: '20260711', page_path: '/x', event_name: 'lead_capture' }, data_api_value: 1 }] }] }, vercelRows: [] });
  assert.equal(rows[0].measurement_status, 'WINDOW_MEASUREMENT_DEGRADED');
});
test('post-CSP conversion window is measured', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R3_event_count_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/x', event_name: 'lead_capture' }, data_api_value: 1 }] }] }, vercelRows: [] });
  assert.equal(rows[0].measurement_status, 'MEASURED');
});
test('A5 rows are blocked', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [] }, vercelRows: [{ day: '2026-07-12', requestPath: '/x', value: 1, source: 'A5', source_family: 'speed_insights' }] });
...
test('release IDs are preserved', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/x' }, data_api_value: 1 }] }] }, vercelRows: [], sourceReleaseIds: { canonical_release_id: 'rel_x', acquisition_release_id: 'run_y' } });
...
test('privacy assertion is explicit', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/x' }, data_api_value: 1 }] }] }, vercelRows: [] });
...
test('a post-CSP window settles after the arrival-lag threshold', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/x' }, data_api_value: 1 }] }] }, vercelRows: [], asOf: '2026-07-16' });
  assert.equal(rows[0].settlement_state, 'settled');
});
test('a recent window remains fresh until the arrival-lag threshold passes', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260714', page_path: '/x' }, data_api_value: 1 }] }] }, vercelRows: [], asOf: '2026-07-16' });
  assert.equal(rows[0].settlement_state, 'fresh');
});
test('page-window settlement rejects a missing deterministic as-of date', () => {
  assert.throws(() => rawJoinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260714', page_path: '/x' }, data_api_value: 1 }] }] }, vercelRows: [] }), /as-of/i);
});
...
test('page-window join preserves each R1 metric and R3 event observation for one page-day', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [
...
test('R3 eventCount falls back to custom-event source policy and retains the canonical BQ value', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R3_event_count_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/x', event_name: 'lead_capture', metric_name: 'eventCount' }, metric_name: 'eventCount', data_api_value: 2, bq_value: 5 }] }] }, vercelRows: [] });
...
test('page-window join rejects explicitly invalid canonical releases', () => {
  assert.throws(() => joinPageWindow({ ga4Release: { release_status: 'INVALID', rows: [] }, vercelRows: [] }), /invalid canonical release/);
});
...
test('joined rows are deterministic in sort order', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/z' }, data_api_value: 1 }, { row_key: { date: '20260711', page_path: '/a' }, data_api_value: 1 }] }] }, vercelRows: [] });
...
test('title variants aggregate rather than retaining an input-order-first value', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [
...
test('non-additive title variants fail closed and remain deterministic across input order', () => {
  ...
  const build = (rows) => joinPageWindow({ ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: rows }] }, vercelRows: [] });
...
test('page-scoped R3 event rows survive independently for different pages', () => {
  const rows = joinPageWindow({ ga4Release: { rows: [{ report_key: 'R3_event_count_daily', sanitized_rows: [
...
test('R7 observations retain distinct FAQ IDs even with metric names', () => { const release = { release_status: 'VALID', rows: [{ report_key: 'R7_custom_event_faq_daily', sanitized_rows: [{ row_key: { date: '20260712', page_path: '/x', faq_id: 'faq-a' }, metric_name: 'eventCount', bq_value: 2 }, { row_key: { date: '20260712', page_path: '/x', faq_id: 'faq-b' }, metric_name: 'eventCount', bq_value: 3 }] }] }; assert.equal(joinPageWindow({ ga4Release: release, vercelRows: [], asOf: '2026-07-20' }).length, 2); });
test('page-window join excludes source records outside the requested bounds', () => {
  const rows = joinPageWindow({
    ga4Release: { rows: [{ report_key: 'R1_pageview_daily', sanitized_rows: [
      { row_key: { date: '20260701', page_path: '/x', metric_name: 'screenPageViews' }, metric_name: 'screenPageViews', data_api_value: 4 },
      { row_key: { date: '20260712', page_path: '/x', metric_name: 'screenPageViews' }, metric_name: 'screenPageViews', data_api_value: 7 },
    ] }] },
    vercelRows: [
      { day: '2026-07-01', requestPath: '/x', metric_name: 'pageviews', value: 4 },
      { day: '2026-07-12', requestPath: '/x', metric_name: 'pageviews', value: 7 },
    ],
    windowStart: '2026-07-12',
    windowEnd: '2026-07-12',
  });
  assert.deepEqual(rows.map((row) => row.measurement_date), ['2026-07-12']);
  assert.equal(rows[0].window_start, '2026-07-12');
  assert.equal(rows[0].window_end, '2026-07-12');
});
...
test('release provenance rejects invalid status and mismatched manifest IDs', () => {
  const selected = { canonical_release_id: 'rel_0123456789abcdef', acquisition_release_id: 'run_0123456789abcdef' };
  assert.throws(() => join.validateReleaseProvenance({ release_status: 'INVALID' }, {}, selected), /VALID/);
  assert.throws(
    () => join.validateReleaseProvenance({ release_status: 'VALID', ...selected }, { canonical_release_id: 'rel_ffffffffffffffff', acquisition_release_id: selected.acquisition_release_id }, selected),
    /match/
  );
});
...
test('release provenance rejects VALID GA4 artifacts that omit either release ID', () => {
  const selected = { canonical_release_id: 'rel_0123456789abcdef', acquisition_release_id: 'run_0123456789abcdef' };
  const manifest = { ...selected };
  assert.throws(
    () => join.validateReleaseProvenance({ release_status: 'VALID', acquisition_release_id: selected.acquisition_release_id }, manifest, selected),
    /GA4 artifact.*canonical_release_id.*required/i
  );
  assert.throws(
    () => join.validateReleaseProvenance({ release_status: 'VALID', canonical_release_id: selected.canonical_release_id }, manifest, selected),
    /GA4 artifact.*acquisition_release_id.*required/i
  );
});
