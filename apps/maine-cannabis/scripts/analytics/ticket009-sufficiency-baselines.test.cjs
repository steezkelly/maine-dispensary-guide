...
function eligiblePeerContext() { return { settlement_state: 'settled', measurement_status: 'MEASURED', window_comparable: true, change_context_evaluated: true, task_contract_status: 'CONFIRMED' }; }

test('exact peer cell is selected when minimum count is met', () => {
  const target = { metric_family: 'gsc_ctr', query_intent: 'how_to', branded_status: 'nonbranded', position_band: '1-3', device: 'mobile', serp_promise_family: 'generic', ...eligiblePeerContext() };
...
test('sparse exact cell falls back explicitly', () => {
  const target = { metric_family: 'gsc_ctr', query_intent: 'how_to', branded_status: 'branded', position_band: '1-3', device: 'mobile', serp_promise_family: 'generic', ...eligiblePeerContext() };
  const peers = [
...
test('task-compatible fallback never admits another primary task family', () => {
  ...
  const target = { canonical_page_path: '/target', metric_family: 'gsc_ctr', query_intent: 'how_to', primary_task_family: 'how_to_task', numerator: 1, denominator: 10, ...eligiblePeerContext() };
...
test('peer selection excludes evidence that downstream investigation would block', () => {
  const target = { canonical_page_path: '/target', metric_family: 'gsc_ctr', query_intent: 'how_to', branded_status: 'nonbranded', position_band: '1-3', device: 'mobile', serp_promise_family: 'generic', numerator: 1, denominator: 10, ...eligiblePeerContext() };
  const badPeers = [
    ...
    { ...target, canonical_page_path: '/unknown-health', measurement_status: 'UNKNOWN', numerator: 7 },
    { ...target, canonical_page_path: '/unevaluated-change', change_context_evaluated: false, numerator: 8 },
  ];
