...
test('R3 Data API query uses the same event population as the BigQuery mirror', () => {
  ...
  assert.deepEqual(body.dimensionFilter.filter.inListFilter.values, ['page_view', 'scroll', 'scroll_depth', 'click', 'page_engaged', 'fa_open', 'faq_open', 'cta_view', 'mdg_action_select', 'mdg_partner_referral', 'lead_capture', 'affiliate_click', 'user_engagement', 'session_start']);
});
...
test('R3 Data API query includes the partner referral event alongside the BigQuery mirror', () => {
  const body = buildReportRequest(REPORTS.R3_event_count_daily, { from: '2026-07-01', to: '2026-07-07' });
  assert.ok(body.dimensionFilter.filter.inListFilter.values.includes('mdg_partner_referral'));
});
