...
if (format === 'json') {
  ...
  md += `\n## How to apply\n\nThis script does NOT touch any .astro file. Output is for operator review.\n\n1. Review the bucket tally — confirm the consumer/operator/hybrid split makes sense.\n2. Spot-check 5-10 titles per bucket against the actual page body (operator names correct, town formatting correct, no truncation issues).\n3. Approve the bucket + template rule (or amend the rule in the script).\n4. Apply per-file patches via the patch tool with auto-generated strings.\n5. Commit the reviewed patches on a named branch.\n6. Run \`node scripts/git/pre-push-verify.cjs --ref=origin/main\` and \`npm run build:isolated\` against that exact candidate.\n7. Push normally with \`git push origin HEAD:refs/heads/$BRANCH_NAME\`.\n8. Wait for Vercel Ready on the exact pushed SHA, then run \`MDG_PREVIEW_URL=https://your-exact-preview.vercel.app npm run verify:post-deploy\`.\n9. Re-measure GSC in 14-28d; expect CTR lift from 0.5-2% baseline toward 2-4% range.\n`;
  out = md;
}
