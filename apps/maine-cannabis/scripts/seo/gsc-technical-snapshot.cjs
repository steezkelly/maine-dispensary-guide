...
function buildSnapshot({ sitemapUrls, manifestRows, coverageRows, redirectSources = declaredRedirects(), pageChecks = {}, prior, extractedAt = new Date().toISOString() }) {
  ...
  const excludedManifestRoutes = [...manifest].filter(key => !indexabilityFor(key).sitemapEligible).sort();
  const routes = [...all].filter(key => indexabilityFor(key).sitemapEligible).sort().map(key => { const inspection = coverage.get(key); const check = pageChecks[key] || {}; const reasons = [];
    ...
    if (check.fetchStatus && check.fetchStatus >= 300) reasons.push('PRODUCTION_HTTP_FAILURE'); if (check.noindex) reasons.push('NOINDEX');
  ...
  return { schemaVersion: 1, snapshotKind: 'indexation-technical', extractedAt, sources: { productionSitemap: true, canonicalRouteManifest: true, gscCoverageExport: coverageRows.length > 0, redirects: redirectSources, excludedManifestRoutes }, routes };
}
...
module.exports = { buildSnapshot, completeCoverage, latestCoverage, route, indexabilityFor, fetchSitemapUrls };
