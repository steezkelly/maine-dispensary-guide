...
function buildSnapshot({ sitemapUrls, manifestRows, coverageRows, redirectSources = declaredRedirects(), pageChecks = {}, prior, extractedAt = new Date().toISOString() }) {
  ...
  const routes = [...all].sort().map(key => { const inspection = coverage.get(key); const check = pageChecks[key] || {}; const reasons = [];
    ...
    if (inspection?.status === 'ERROR') reasons.push('GSC_CRAWL_FAILURE'); if (inspection?.status === 'NOT_INDEXED') reasons.push('GSC_NOT_INDEXED'); if (inspection?.status === 'NEUTRAL') reasons.push('GSC_NEUTRAL_REQUIRES_REVIEW');
    if (check.fetchStatus && check.fetchStatus >= 400) reasons.push('PRODUCTION_HTTP_FAILURE'); if (check.noindex) reasons.push('NOINDEX');
    if (check.canonical && check.canonical !== `${SITE}${key}`) reasons.push('CANONICAL_POINTS_ELSEWHERE'); if (!inspection) reasons.push('GSC_COVERAGE_UNOBSERVED'); if (check.checked === false) reasons.push('PRODUCTION_HTML_UNOBSERVED');
