...
|**Last updated: 2026-06-07 EDT** (Sprint 78 + 79 observability: real regression caught, observability layer proven. **#19 Build break** — Sprint 78 hardcode-sweep introduced `import { getSiteStats } from '../../lib/site-stats'` in `maine-cannabis-market.astro` and `maine-dispensary-how-to-open.astro`, but the supporting `apps/maine-cannabis/src/lib/site-stats.ts` and `data/site-stats.json` files were not committed alongside. `astro build` failed with "Could not resolve '../../lib/site-stats'". `check:build-warnings` caught it (CSS build warnings +1 regression: 23→24), and `sprint-score.cjs` flagged "Hub header claim matches reality" as FAILED. Without the observability layer from Sprint 76-77, this would have shipped to production. **Fix** in `e238f8c`: re-created `src/lib/site-stats.ts` (60 lines, `getSiteStats()` accessor with typed `SiteStats` interface) and `src/data/site-stats.json` (canonical 187/65/$246M/1.4M numbers + `currentOcpLicenseeRoster` block). Build green, 10/10 sprint-score checks pass, 223/223 smoke-200. **#20 Dynamic injection proven end-to-end** — bumped `activeAdultUseRetailStores` 187→999 in `site-stats.json`, re-built, all 3 rendered locations in `index.html` (stat-value="999", aside-stat="999", "With 999 active stores") reflected the new value, reverted to 187. The site-stats.ts → getSiteStats() → .astro interpolation pipeline is observably correct: any future drift in the OCP data flows through one source and propagates everywhere automatically. **#21 OCP refresh automation + JSONL audit log** — `apps/maine-cannabis/scripts/ocp/refresh-site-stats.cjs` (~200 lines) wraps `scripts/ocp/fetch-ocp-towns.py`, dedupes, compares to stored, writes new numbers + appends JSONL to `public/data/ocp-stats-history.jsonl`. Modes: `--check` (verify), `--dry-run` (show diff), default (write + log). First test surfaced a real divergence: stored 187 (2025 Annual Report, all 343 active AU establishments) vs live deduped 107 (current OCP licensee-search CSV, Store-type only) — both now tracked separately with the methodology difference explained in `currentOcpLicenseeRoster.note`. **#22 Sprint-score 8/8 → 10/10** — added 2 new checks: `OCP stats roster freshness` (warns at >30d, fails at >90d) and `llms.txt freshness` (checks for "Last regenerated" stamp). The check layer now catches the kind of subtle drift that the hand-edited Hub could never catch. **State**: 10/10 pass, 0 failed, 2 warnings (OCP stats 68d old, llms.txt no stamp — both expected). The empirical red→green was the regression caught above — proves the gate works.)
