...
|**Last updated: 2026-06-07 EDT** (Sprint 77 observability bulk close — 6 of the 12 gaps from the 2026-06-07 MDG tracking audit addressed in one batch. **#5** 404.html city count 40+ → 109+ via new `apps/maine-cannabis/src/lib/page-stats.ts` (single source of truth for filesystem-derived counts, used by 404.astro + site-health.astro + future consumers). **#6** new `apps/maine-cannabis/scripts/admin/sprint-score.cjs` (~340 lines) — 8-check health snapshot (dist, sitemap, content-health regression, broken image refs across heroes/infographics, git tree, Hub claim consistency). 8/8 pass, exit codes: 0=ok, 1=fail, 2=warn. The "100/100" header claim is now machine-verifiable — `sprint-score` exits 1 if the Hub claim is inconsistent with the data. **#7** `public/status.json` served live at `https://mainedispensaryguide.com/status.json` — written by `vercel-build.sh` BEFORE the astro build so it's copied verbatim to dist (initial fix had it after the dist cp, which would have missed it; rewired to the right step). 1764 bytes, status "warn" (8/8 checks pass, 1 warning from 22 uncommitted files — expected for in-progress session). **#8** IndexNow submission log — `apps/maine-cannabis/scripts/submit-indexnow.cjs` now appends JSONL to `public/data/indexnow-log.jsonl` (served at `/data/indexnow-log.jsonl`) on every successful OR failed submission. First test: HTTP 200, 221 URLs, log entry created. **#9** new `apps/maine-cannabis/scripts/build/smoke-200.cjs` (~150 lines) — HEAD-200 check for all 223 published pages against `MDG_BASE` (default mainedispensaryguide.com, override for preview), concurrency=8, ~5s. Live result: 223 ok, 0 redirects, 0 broken. Catches the case where a build goes green but a specific page 404s. **#12** archived 6 truly-orphaned docs (`ROADMAP_V1_DRAFT.md`, `ROADMAP_ULTIMATE_EDITION.md`, `ROADMAP_FOUNDERS_BIBLE.md`, `ROADMAP_BIBLE_V1.md`, `ROADMAP_CONTENT.md`, `SESSION_SUMMARY.md`) to `docs/archive/2026-04-roadmap-drafts/` via `git mv` (history preserved) with a `README.md` explaining the move. Updated 4 references in `ORPHANED_TASKS_REPORT.md` to point to the new locations. Rewrote `PROJECT_STATE.md` and `MISSION_CONTROL.md` — both were 40+ days stale with placeholders (PROJECT_STATE said "15 Maine cities" (real: 109); MISSION_CONTROL said "Total Authority Guides: 0" (real: 150+), "Lead Gen: activate PDF form" (already done in Sprint 77)). All checks verified: build green (4.7s), sprint-score 8/8, smoke-200 223/223, status.json regenerated, 404.html 109+. **Combined with Sprint 76 and 77 lead-capture**: every published page now produces a machine-readable health signal (status.json + IndexNow log + 224-page smoke), every form submission produces a GA4 conversion event, and the build is regression-guarded at 3 layers. The 5 remaining audit gaps (#11 GSC ingestion, #18 form completion dashboard, #19 OCP-driven site-stats.json, plus small polish items) are queued for Sprint 78+.)
