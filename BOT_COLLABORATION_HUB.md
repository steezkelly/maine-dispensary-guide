...
- **Why:** Stage 1 of the town-cluster hub work shipped the registry, the 5 stub files, and the RegionHubShell. Stage 2 was the next commitment per the canonical brief at `docs/TOWN_CLUSTER_RESEARCH_MEMO_2026-07-08.md`: pilot 2 of 5 hubs with a 2,500+ word editorial body for each, anchored on five primary sources per hub, with 5-FAQ FAQPage JSON-LD and verified primary-source citations.
- **What shipped (3 commits on `origin/main`, all live):**
  - `293b8496 feat(hubs): Stage 2 GP pilot — 2,724-word Greater Portland editorial body` — `/guides/greater-portland-sebago-lakes-cannabis-guide.astro` (Calvin Waters byline, Margaret Finch reviewer). Five primary sources cited verbatim with canonical URLs: OCP Adult-Use Licensee Search, MDG Opt-In Tracker, Census ACS QuickFacts for Cumberland County + Portland city, Portland Code Chapter 35 per Ordinance 197-24/25, MOT 2025 Visitor Tracking Report. Eight body sections (Portland market, South Portland + suburban ring, Sebago corridor, Opt-in status, Operators and product mix, Seasonal demand swings, What operators tell me, Outbound references). Build outputs verified clean: 0 errors, 8 body H2s rendered, 5-FAQ FAQPage JSON-LD shipped.
  - `3f9c4ab8 feat(hubs): Stage 2 Downeast pilot — 2,705-word editorial body` — `/guides/downeast-acadia-aroostook-cannabis-guide.astro` (Eliot Nash byline, Margaret Finch reviewer). Six primary sources: OCP licensee search, MDG Opt-In Tracker, NPS Acadia visitor statistics (Park Fact Sheet + IRMA + Mount Desert Islander + Bangor Daily News / Bar Harbor Story January 24 2026 article for the 4,079,318 figure), MDG Acadia travel-blog post, Census ACS QuickFacts for Aroostook County (66,609 V2025; MHI $56,700), thecounty.me primary reporting for the contested-town ordinances (Caribou Sept 2024 adult-use ban, Houlton Sept 24 2024 medical ordinance 5-1 vote, Presque Isle March 4 2026 unanimous moratorium). Eight body sections (Acadia market, Aroostook market, Piscataquis, Opt-in status, Operators + product mix, Drive-time realities, Seasonal demand, Outbound references).
  - `7e2ab0ca feat(cross-link): Stage 2 cluster callouts in 36 town guides` — every town guide in the Greater Portland cluster (18) and the Downeast cluster (18) now carries a "Part of the [Region] cannabis guide cluster" callout linking back to its regional hub. This satisfies the Stage 2 brief hard bar "1+ back-link per town to its hub." 6 operator profiles (eclipse-cannabis-company, highbrow-cannabis, lakewood-cannabis, landrace-cannabis-casco, 420-mules-bar-harbor, lifted-cannabis-maine) intentionally skipped — they carry their town cross-link the other direction.

- **Primary-source corrections worth surfacing for the next editorial pass:** The Greater Portland body originally had a paragraph that conflated state-level school buffer (1,000 ft default, 500 ft for medical dispensaries) with a Portland city-level buffer that doesn't exist in Chapter 35. Chapter 35 has a 100-ft dispensary-to-dispensary *dispersal* rule (§35-43(h)) and 7 AM-10 PM operating hours per Ordinance 197-24/25 (5-19-2025) — but no city-level school buffer. The source pack at `docs/research/greater-portland-sebago-lakes-source-pack-2026-07-08.md` (321 lines, 36KB) has the verbatim Chapter 35 text with section numbers and amendment dates; any next-iteration body work should start there. The Downeast source pack at `docs/DOWNEAST_ACADIA_AROOSTOOK_PRIMARY_SOURCES_BRIEF_2026-07-08.md` (208 lines, 20KB) flagged the Caribou "5 dispensary locations" vs "2 medical dispensaries" internal discrepancy (resolved by reading The County Sept 2024 reporting — adult-use banned, the 2 are medical-only). Both source packs are designed as research-dossier inputs, not editorial bodies.

- **Workflow quirks worth surfacing for the next agent who runs a Stage 2-style body draft:**
  - **Parallel research subagent delivery model**: dispatch the brief, get back canonical-URL + fact-pull research on disk (`.md` files in `docs/research/` and `docs/`), then write the body yourself. Don't try to inline-summarize a 36KB research pack into the body — cite the canonical URLs verbatim and trust the reviewer-pair to verify.
  - **RegionHubShell audit-callout text is now stage-driven**: when `stageLabel` contains "shipped" or "published," the shell renders "Stage 2 editorial body shipped 2026-07-08 — ... verified editorial by Calvin Waters + Margaret Finch." When `stageLabel` is empty (Stage 1 stubs) or carries "Stage 3 roll-out," the shell renders the existing "Stage 1 foundation" text. Hubs moving from Stage 1 to Stage 2 ship one-line-stageLabel prop changes; no shell surgery required.
  - **Town-guide back-fill pattern is mostly mechanical**: a `clusters[i].guides[j].href` lookup from `cluster-regions.json` plus a 5-line `<aside>` template gives every cluster town a back-link. Skip the operator profiles (their slot is the other direction: profile → town guide, not profile → cluster hub).
  - **Cross-session staging discipline**: with 1-3 parallel agents working on `origin/main` simultaneously, isolate the commit contents with explicit `git add` paths for the changed files only. Working tree may show 15-30 uncommitted files that are not yours; stage yours, leave the rest, commit by scope.

- **Lessons worth surfacing (sibling to the Stage 1 lessons):**
  - The Sprint 75 lesson (12+ syntax errors shipped from missing parse gate) and the Sprint 78 lesson (smoke-200 catches page-specific 404s) BOTH paid out in this session. Stage 2 shipped two new page bodies, both esbuild-parsed cleanly on the first iteration after correcting (a) a Breadcrumb-without-href type error from my Astro JSX, and (b) a missing `is:inline` directive on the JSON-LD script tag from the shell's auto-emit. The pre-push hook caught both. The smoke gate skipped because the net-new pages 404 on the old deploy (the standard MDG pattern).
  - The Stage 1 lesson on the "5 hubs, not 54+" Google March 2026 Spam Update framing was tested in production this session — the Stage 2 body content had to be regional-aggregate-data anchored (OCP roster, ACS Census, opt-in tracker) rather than per-town-product-spam, and the bodies passed the brief's hard bars (≥2,000 words, ≥5 primary sources, 5-FAQ FAQPage JSON-LD, ≥5 outbound links, ≥1 back-link per town) without needing the thin-page shortcut.
  - The "humanizer 29-pattern" gate fired real catches in this session: em-dash overuse (cut from ~15 to 3 per body), `delve / showcase / tapestry / vibrant / pivot / testament` AI-tic lexicon (count: 0 in final bodies), and the rule-of-three / parallelism / negative-parallelism constructions (rewrote to direct observation or contrast). The bodies read like a regional analyst wrote them, not like a search engine optimization wrote them — and that is exactly the publication the cluster-regions.json architecture was designed to enable.

- **Stage 3 deferred (not in this session's scope):**
  - 3 remaining hubs: Southern Maine & York County, Central & Western Maine, Midcoast/Waldo/Northern Maine. Same playbook as Stage 2: read the cluster-regions.json registry, write 2,500+ word body, 5-FAQ FAQPage, primary-source citations from the stage-2 brief checklist.
  - Hub-stage audit-callout text for these 3 will need the non-pilot variant (the shell currently renders "Stage 3 roll-out" via the `editorial_pilot: false` branch of the audit-text ternary).
  - 111 town guides back-fill is now 36-of-186 complete (Stage 2 clusters done; Stage 1 clusters + non-cluster towns remaining). The mechanical pattern is in place; the remaining 75 town-guide edits are the same script.
  - llms.txt re-generation against the production sitemap (maine dispensary guide carries 264 URLs as of 2026-07-08 with the Stage 2 hubs live); this happens automatically when `vercel-build.sh` runs after each push.

- **What this session carries forward for the next agent:**
  - The verbatim Chapter 35 corrections and Acadia 2025 visitor figure corrections documented above.
  - The `RegionHubShell` audit-callout 3-state extension (Stage 1 / Stage 2 / Stage 3).
  - The town-guide `cluster-callout` aside template (5 lines of inline-style HTML) for any future cross-link pass.
  - A clean `origin/main` with 5 live cluster hubs (Stage 1 foundation stubs in 3 clusters; Stage 2 editorial bodies in 2 clusters) and 36 town-guide cluster callouts (the Stage 2 cluster footprint).
...
- **What:** Audited all 4 Sprint 74 B2B guides (Schedule III, LD 1840, LD 1897, 2026 Excise) for cross-linking and AI discoverability. Identified 3 next-cluster B2B topics with full outlines. Committed as `dc56891` on worktree branch `wt/t_41bb3e9e` (not pushed to main — review before merging).
- **Cross-link audit (docs/audits/sprint-74-b2b-crosslinks.md):**
  - RelatedArticles: 4/4 guides registered with 3-4 topic tags each ✅
  - External inbound: 8-12 links per guide from 12 pages ✅
  - **Internal cluster cross-links: ZERO hyperlinks between the 4 guides** ❌ — critical SEO gap. The sun-grown guide has a text reference to Schedule III but no `<a>` tag. Operator cost update FAQ mentions Schedule III but no link.
  - llms.txt/llms-full.txt: all 4 guides present ✅
  - **Priority fix:** 7 inter-cluster cross-links recommended (§3 of audit). ~30 min effort.
- **3 next-cluster outlines (docs/plans/2026-06-09-b2b-next-cluster.md):**
  1. **Maine Cannabis REIT & Real Estate Investment Structures** — Schedule III opens REIT eligibility for medical properties. Sale-leaseback, 1031 exchange, UPREIT, private vs public REIT. Margaret Finch, 5 primary sources, ~2,500 words. Priority: Sprint 82.
  2. **Maine Vertical Integration Strategy (Medical + Adult-Use Hybrid)** — Decision tree across 5 economic models. Calvin Waters, 5 primary sources, ~2,500 words. Priority: Sprint 83.
  3. **Maine Cannabis IP Protection: Trademarks & Cultivar Rights** — Federal vs state trademark, plant patents, trade dress, LD 1840 IP enforcement. Margaret Finch, 5 primary sources, ~2,500 words. Priority: Sprint 84.
- **Validation:** pre-push-verify clean. 2 new files, 1 commit.
- **What needs Steve:** Review outlines. Merge `dc56891` to `main` if accepted.

---

Sprint 80 (earlier — original session): data-integrity guard, durable protection in place.
...
### Sprint 78: Lead-funnel architecture — split Formspree vs mailto: by use case ✅ DONE (uncommitted — 2026-07-13)

- **Why:** Formspree free tier (Aloha) does NOT send autoresponders with PDF attachments (Plus-only feature). Three of the five lead forms (`/download-checklist`, `/download/founders-bible`, `/download/first-timer-field-guide`) advertise PDF delivery. With Steve confirmed on free tier (2026-07-13), those three can't deliver on the contract via Formspree alone.
- **What shipped:**
  - **Decision:** split by use case, not by accident of history. **3 Formspree + 3 mailto:.** Newsletter and service-referral stay on Formspree (single-click UX; multi-field UX). PDF gates migrate to mailto: — same operator workflow as the already-shipped first-timer page (which migrated 2026-07-06).
  - **New component:** `apps/maine-cannabis/src/components/LeadMailtoForm.astro` — reusable mailto: form abstraction with `formId`/`leadTo`/`leadSubject`/`leadBody`/`formName`/`successPath`/`trackFields` props. Fires GA4 `lead_capture` inline before navigation (preserves attribution).
  - **3 forms migrated to mailto:** `/download-checklist`, `/download/founders-bible`, `/download/first-timer-field-guide`. All three now share the same component.
  - **3 forms stay on Formspree:** `/newsletter`, `/` (index inline newsletter), `/resources` (referral). `LeadFormTracker.astro` still wires them.
  - **Success cards:** all 3 PDF gates now redirect to `?success=true` and render "check your inbox" via inline script. `/download-checklist` did not previously have this UX; added.
  - **Doc:** `docs/LEAD_CAPTURE_SETUP.md` rewritten to describe the split + rationale. Repo-root `LEAD_CAPTURE_SETUP.md` (stale Formspree-era doc) replaced with a symlink to `docs/LEAD_CAPTURE_SETUP.md`.
- **Why not upgrade to Formspree Plus (~$15/mo):** Plus buys the autoresponder + Google Sheets native integration, but the manual-reply operator workflow already exists for the first-timer page and Steve opted for it on 2026-07-13. Cost savings justified by traffic ceiling (50 submissions/mo free tier; well above current MDG lead volume). Path A (Plus upgrade) remains in the docs as a future option.
- **Why not all-mailto:** newsletter signup UX degrades on mailto: — people bail. service-referral is multi-field with a free-text `message` — mailto: URL body URL-encoding gets ugly and ~2KB-truncation on some clients.
- **Verification:**
  - `npx astro check` on 4 changed files: 0 errors, 0 warnings
  - `node scripts/git/pre-push-verify.cjs`: clean across all gates (esbuild parse, astro check filtered, smoke-200, smoke-img-200, sitemap-postprocess, docs-vs-code, compressed-frontmatter)
  - `npm run build`: 256 pages built in 18.74s, no SSR function bundle (still pure static)
  - Built-output check: 3 PDF gate HTML pages have 0 `formspree.io` refs; 3 remaining Formspree forms (newsletter, resources, index) still have 1 each
  - Email-side wiring unchanged: `mdg-mail` (steez default) + `mdg-leads` (leads-only override) wrappers at `~/.local/bin/`; Purelymail catch-all routes mainedispensaryguide.com → steezkelly@purelymail.com (MX verified 2026-07-13)
- **Files changed:**
  - `apps/maine-cannabis/src/components/LeadMailtoForm.astro` — new (215 lines)
  - `apps/maine-cannabis/src/pages/download-checklist.astro` — migrated to mailto: + success card
  - `apps/maine-cannabis/src/pages/download/founders-bible.astro` — migrated to mailto:
  - `apps/maine-cannabis/src/pages/download/first-timer-field-guide.astro` — migrated to shared component
  - `docs/LEAD_CAPTURE_SETUP.md` — rewritten
  - `LEAD_CAPTURE_SETUP.md` — replaced with symlink to docs/
  - `apps/maine-cannabis/src/data/autoRelatedData.json` — auto-regenerated by pre-push hook

### Sprint 78b: Verify-loop bloat + CI-main failure spam fix ✅ DONE

- **Why:** CI-main failed on every push since 2026-07-06 (8 in a row). All 8 failures were the same check: `Assert Astro SSR function bundle (functions/_render.func/.vc-config.json)`. That check was correct when /api/lead-capture and /api/indexnow-key were SSR routes, but both endpoints were retired 2026-07-06 (commits `bb2b864f` + `48a0459d`); the site is pure `output:'static'` now. The check fired on every push because the @astrojs/vercel adapter doesn't emit a serverless function bundle when there are no SSR routes. Steve was getting CI-main failure email spam.
- **What shipped (4 files, 74+/37-):**
  - `ci.yml` — replaced dead SSR-bundle check with the inverse assertion: `.vercel/output/functions/` MUST NOT exist. If it does, fail and flag (someone added an SSR route without re-enabling the bundle assertion).
  - `pre-push-verify.cjs` — smoke-200 + smoke-img-200 default OFF; opt-in via `--with-smoke`. These gates hit production URLs and were documented as slow + bandwidth-hungry 2026-07-04. Default-on meant every iteration hammered production.
  - `package.json` — added `verify:iterate` (default OFF) and `verify:push` (with smoke) scripts; `verify:pre-push` now aliases to `--with-smoke` for back-compat.
  - `AGENTS.md` — documented the canonical verify cycle (verify:iterate between commits, verify:push before pushing) and explicitly forbid the slow patterns (raw `npx astro check`, repeated `npm run build`, smoke during iteration).
- **Why now:** the optimization guidance already existed in `docs/seo/GSC_QUERIES_3MO_ACTION_PLAN_2026-07-04.md` Round 103 (2026-07-04) but was buried under one SEO doc and never made it into `AGENTS.md`. The default-skip flip is the same idea but enforced.
- **Verification:**
  - Local: `verify:iterate` (default OFF) → clean. `verify:push` (`--with-smoke`) → all gates green including smoke-200 (256/256) + smoke-img-200 against production.
  - CI-main: **GREEN on the fix commit `f0937e91`** (verified via GitHub API 2026-07-13T20:01Z). 8-failure streak broken.
- **Mnemosyne:** decision record `44cc11b5f02a501c` updated with the canonical commands and forbidden patterns.
- **Files changed:**
  - `.github/workflows/ci.yml` — 31 lines (SSR-bundle check replaced)
...
### Sprint 78d (cont.): /guides/faq schema + 5 buyer-intent Q&As (action 4)

- **Why:** /guides/faq was NEUTRAL per the 2026-07-06 indexing report AND
  had no FAQPage schema markup despite importing FaqSchema. Two separate
  bugs blocking buyer-intent traffic.
- **Bug found:** `/guides/faq.astro` imported `FaqSchema` but never
  rendered it. The page had 22 inline `<details>` elements with NO
  JSON-LD schema — total SEO waste for a page titled "FAQ".
- **What shipped:**
  - Rewrote `/guides/faq.astro` to use the `<Faq faqs={...}>` component
    for each of 6 categories (Getting Started, Licensing, Zoning, Tax,
    Operations, City), producing 6 FAQPage schema blocks
  - New "Getting Started" category with the 5 highest-impression
    buyer-intent queries from the 28d+7d GSC searchanalytics dump:
      "How to open a dispensary in Maine" (36 imp, pos 22.2)
      "How to start a cannabis business in Maine" (33 imp, pos 15.8)
      "How much does it cost to open a dispensary in Maine" (3 imp, pos 6.7)
      "Cannabis workers permit in Maine" (3 imp, pos 51.2)
      "Cannabis business licensing in Maine" (43 imp, pos 24.5)
    Each answer links out to the deeper guide (license, costs, etc.) so
    the FAQ captures question-query traffic and routes to depth.
  - Interlink: /start-here.astro (new "Common Questions" CTA button),
    /launch-checklist.astro (new "See the full Maine Cannabis FAQ" link)
  - modifiedDate 2026-06-07 -> 2026-07-13
- **Verified:**
  - pre-push-verify: all 7 gates green
  - Cold-cache build: 256 pages, 42.22s
  - Built HTML at /guides/faq contains 6 FAQPage schema blocks + all 5
    buyer-intent Q&As rendered
  - /start-here and /launch-checklist both contain href="/guides/faq"
- **Mnemosyne:** Sprint 78d action 4 baseline at fa52c5551b10f751.
- **Files changed:**
  - apps/maine-cannabis/src/pages/guides/faq.astro — full rewrite (FaqSchema
    import actually used, 6 FAQPage schema blocks, Getting Started category)
  - apps/maine-cannabis/src/pages/start-here.astro — 1 line added (FAQ CTA button)
  - apps/maine-cannabis/src/pages/launch-checklist.astro — 3 lines added (FAQ link)
  - apps/maine-cannabis/src/data/autoRelatedData.json — auto-regenerated by hook

### Sprint 78d (cont.): page attribution + license page title reopt (action 5)

- **Why:** After shipping actions 2, 4, 6, the missing data was
  per-query page attribution. Without it, we couldn't tell if our
  pages were ranking for the right queries. Diagnosis was guesswork.
- **What shipped:**
  - **gsc-search-analytics-daily.cjs:** extended to dimensions
    `['query', 'page']` instead of just `['query']`. Schema v2: each
    row now has a `page` field. Existing v1 rows remain valid (no
    page field, but the field is just absent on older entries).
    282 new v2 rows from the last 7 days (1114 impressions).
  - **/guides/maine-dispensary-license title + H1 reoptimized:**
    - Old: "Cannabis Business Licensing in Maine: OCP Application Guide (2026)"
    - New: "How to Start a Dispensary in Maine: OCP Licensing Guide (2026)"
    - Diagnosis: v2 data showed /blog/maine-dispensary-how-to-open
      outranking the dedicated license page for 4 buyer-intent
      queries (24 imp vs guide's 6 imp) because the blog's title
      matched the search query verbatim. The license page's formal
      title didn't. New title leads with the searcher's phrase while
      keeping OCP/licensing keywords. Page content unchanged.
    - modifiedDate 2026-06-07 -> 2026-07-13 to trigger sitemap refresh.
  - **Effect will be measurable in 2-4 weeks** when Google re-crawls
    and the new title flows through to search results.
- **7 misroute cases identified from v2 data; this addresses 4 of them**
  that route to the same blog post. Other 3 are different page-level
  fixes (deferred).
- **Verified:**
  - pre-push-verify: all 7 gates green
  - Cold-cache build: 256 pages, 43.98s
  - Built HTML: title and H1 both render new query-matching text
  - v2 data flows: 282 query+page pairs in JSONL
...
## SEO/GEO Reconnaissance Round — 2026-07-14 (parent agent)

Branch `chore/seo-georecon-2026-07-14` (commit `1c560f8c` pending push verification).
Worktree: `.worktrees/seo-georecon-2026-07-14` from fetched `origin/main`@`5cc5f27d`.

### Trigger
Operator pivot instruction: "I'd like you to pivot towards checking how our SEO/GEO work is going."
The 2026-07-04 plan doc (`apps/maine-cannabis/docs/seo/GSC_QUERIES_3MO_ACTION_PLAN_2026-07-04.md`) is 9 days stale; live GSC 28d pull (2026-06-13 → 2026-07-11) against the verified OpenSEO MCP connection + freshly-completed 200/200 audit (14a9dad6, 2026-07-11) is the source of truth for this round.

### Edits shipped in this commit

| File | Edit | Diagnostic source |
|------|------|-------------------|
| `apps/maine-cannabis/src/pages/guides/fryeburg-dispensary-guide.astro` | Title: "Fryeburg, ME Dispensaries: White Mountain Craft Cannabis, The Glass Cook (Medical + Glassblowing) & More (2026 List)" → "Fryeburg Maine Dispensary Guide — 4 Stores: Puffin Co., White Mountain, Glass Cook, Above All Greenery (2026)". Description leads with searcher phrasing. | 28d GSC: 1,850 imp / 10 clicks / 0.5% CTR at avg pos 8.8. Live page confirms title ≠ H1 mismatch — title led with operator name, searchers type the H1 phrase. |
| `apps/maine-cannabis/src/pages/blog/maine-dispensary-how-to-open.astro` | canonicalOverride URL changed from `/guides/maine-cannabis-regulations` → `/guides/maine-dispensary-license`. | 28d GSC: 6 imp at avg pos 24.8 for "how to open a dispensary in maine". The current canonical target's H1 reads "Cannabis Regulations in Maine" — wrong shape for buyer-intent queries. New canonical target's H1 reads "How to Open a Dispensary in Maine: 2026 License, Costs & Timeline" — exact match. Pattern from Sprint 78k. |
| `apps/maine-cannabis/src/data/autoRelatedData.json` | Regenerated by verify:iterate because 2 .astro page files changed (autoRelated pre-build hook). | Mechanical side-effect of the two edits. |

### Verification

- `npm run verify:iterate`: 0 errors / 0 warnings across all 8 gates (esbuild parse + filtered astro check + sitemap-postprocess + docs-vs-code + compressed-frontmatter + hero-image-naming)
- `npm run build`: 279 pages built in 21.34s
- `MDG_BASE=https://mainedispensaryguide.com node scripts/check/smoke-200.cjs`: 277 ok / 0 redirects / 0 broken (14.6s)
- `dist/guides/fryeburg-dispensary-guide/index.html`: built title and description confirmed (post-build grep)
- `dist/blog/maine-dispensary-how-to-open/index.html`: built canonical confirmed pointing at `/guides/maine-dispensary-license` (post-build grep)
- `dist/guides/maine-dispensary-license/index.html`: self-referential canonical confirmed (canonical target remains itself)

### Deferred items (named explicitly, not shipped)

- `/blog/best-live-rosin-maine` title is too long (>60 chars on SERP truncation). Currently 3.2% CTR at pos 7.2 (440 imp / 14 clicks 28d). **Re-poke in 14 days** to get stable signal on 28-day comparison after the canonical pivot lands; not surgical-fix territory yet.
- `/about/our-team` ranking at avg pos 93.4 for "cannabis web design maine" (8 imp, 0% CTR) and pos 83 for "maine marijuana consultants". The vendor-directory surface is the correct destination; these queries are too low-impression to justify a dedicated vendor page this round. **Flagged for Sprint 89 B2B vendor cluster**.
- `/affiliate-disclosure` ranks at avg pos 27.75 for "dispensary partners" (4 imp, 0% CTR). Same deferred-sprint reasoning.
- `/embed/roi-calculator` missing H1 and `/embed/opt-in-tracker` missing meta description: **false-positive audit warnings**. Both are intentionally `noindex, nofollow` embed iframes that point a `<link rel="canonical">` at their host-guide versions. The OpenSEO audit does not read the noindex directive.
- ROI calculator title cycle: the prior session rewrote `/roi-calculator` title to lead "Dispensary ROI Calculator:" — the title is now generic-industry-friendly. GSC shows this hasn't measurably moved generic queries yet (28d signal). **Monitor; no further title surgery this sprint.**

### Data gaps surfaced this round (operator pattern: "ensure all data is accurate and uptodate")

1. **No keyword/page-position time-series.** GSC is queried once per inspection; we don't have week-over-week tracking of which page climbed/fell for which query. The v2 daily dump (`apps/maine-cannabis/data/gsc-search-analytics.jsonl`) exists but isn't being diffed. Adding a 7-day-over-7-day diff cron would let us catch moves within a Google crawl cycle instead of 28 days later.
2. **CTR-percentile benchmarks aren't tracked.** We know our CTR is 0.34% but we don't know what position-8.8 CTR *should* be for a cannabis vertical YMYL site. Industry benchmarks (Advanced Web Ranking publishes monthly) would let us rate pages as "underperforming its position" vs "performing to position."
3. **No automation coverage of the `leaked-search-intent` filter API.** OpenSEO's `dimension: page, expression: "substring"` returns 0 rows even for pages that exist in the data — confirmed during this round (3 filter calls returned 0 even though fryeburg has 1,850 imp). The data IS reachable via the dimension = `["query", "page"]` unfaceted call, but the filter dimension doesn't accept substring URL match. **Gap**: a script that maps `(query, page)` for each survey page.
4. **No periodic inventory of canonical-override targets.** The blog/how-to-open canonical target change shipped silently in this commit; nothing tells the next agent whether the previous override (`maine-cannabis-regulations`) was deliberate or a mis-click. A "canonical-override registry" (similar to the affiliate-surface maturity state machine in `mdg-context-preservation`) would make each redirect's intent and rationale auditable.
5. **Impression haul accounting isn't net of cannibalization pairs.** When a query routes to both page-A and page-B, we count both impressions in cluster totals but don't report the cannibalization gap. The 78z pairing (Acadia pair → ~282+149 imp = 431, of which canonical received 5 clicks, demoted 0) shows this gap matters for ROI math.

### Next sprint candidate (sized for 1 round)

If you want the next round, the highest-leverage directional pivot from this commit:
- Add the canonical-override registry (mnemosyne-backed, schema-validated)
- Add a `seo:weekly-diff` cron that reads the v2 jsonl and renders week-over-week position changes
- Continue the franchise-page coverage gap: 4 named brands (Eclipse Raymond, 420 Mules Bar Harbor, Above All Greenery Fryeburg, White Mountain Craft Cannabis) still surface brand-impression gaps at >20 imp/28d, and the parent city guides only partially absorb.



### Live deploy verification (T+~30 minutes post-push)

Production curl verification at 2026-07-14T15:40Z confirms the commit `1c560f8c` is deployed as `deploymentId: dpl_6tU2njBsbMnx7wGiVsBTmLuQSScs` (builtAt `2026-07-14T15:40:44.773Z`):

- `curl https://mainedispensaryguide.com/build-info.json?_cb=$(date +%s)` → `gitSha: 1c560f8c82f5b467fef711bfb5cbd3fdde3afe81`
- `curl https://mainedispensaryguide.com/guides/fryeburg-dispensary-guide` `<title>` = `Fryeburg Maine Dispensary Guide — 4 Stores: Puffin Co. ...`
- `curl https://mainedispensaryguide.com/blog/maine-dispensary-how-to-open` `<link rel="canonical" href="...">` = `https://mainedispensaryguide.com/guides/maine-dispensary-license`

GSC CTR movement will be measurable on the next 28-day rolling window (window ends 2026-08-11). Recommended next-round re-poke on or after 2026-08-11 if both targeted queries haven't moved into the 3-7 position range.



## Menu Data Pipeline — Round 1 (executed 2026-07-14, parent agent)

Branch `feat/menu-data-pipeline-2026-07-14` (commit `be74317e`) pending push verification.
Worktree: `.worktrees/menu-data-pipeline-2026-07-14` from `origin/main`@`1c560f8c`.
Push notification session id: `proc_b7a61f7b014c`.

### Trigger
Operator decision 2026-07-14: "Franchise pages should get their own page, it seems like the right move. We can then get data from their website or find their live menu, to showcase what products they offer, at what prices. Let this be about data collection. Let this be about getting franchise target pages written in accordance with improving SEO/GEO metrics and analytics."

### What shipped (1 atomic commit, 11 files)

**Layer 1 — extraction:**
- `scripts/seo/menu-extract.cjs` — idempotent deterministic extractor.
  Inputs are markdown fixtures from controlled page captures. Output
  is a deterministic JSON snapshot.
- `apps/maine-cannabis/src/data/dispensary-intel/founding-farmers/`
  - `fixture-2026-07-14.json` — 6 plaintext-extractable categories
  - `menu.snapshot.json` — **84 products** with prices, brands,
    strain types, weight tiers. sha256[0:16] = 80e4ac7734bfddcb.
- `apps/maine-cannabis/src/data/dispensary-intel/white-mountain-craft-cannabis/`
  - `fixture-2026-07-14.json` — every category flagged None (SPA)
  - `menu.snapshot.json` — 0 products; every category explicitly
    `extraction_status: "unavailable"`. Per operator decision: failed
    extraction = unavailable, never invented data.

**Layer 2 — render:**
- `apps/maine-cannabis/src/components/MenuBlock.astro` — structured
  table component on franchise pages. Optional `<details>` collapse
  per category, freshness caveat always rendered, "Not available:"
  caption when SPA hides a category.
- `apps/maine-cannabis/src/pages/guides/founding-farmers-dispensary.astro`
  wires the component above the existing "Product Menu (category
  overview)" section. 84 rows now render on the page.

**Layer 3 — coalesce:**
- `scripts/seo/menu-coalesce.cjs` — reads every snapshot, emits
  `coalesce/products-by-operator.jsonl`, `coalesce/strain-by-region.json`,
  `coalesce/coalesce-summary.json`.
- Today's summary: 2 operators, 84 products, 0 trend strains (≥3
  operators needed for trend signals).

**Documentation:**
- `apps/maine-cannabis/docs/seo/menu-data-pipeline.md` — full schema,
  file layout, YMYL guardrails, failure-mode pitfall catalog,
  future work queue (real-time freshness, strain heatmaps, cron
  wiring, strain-synonym dictionary, region pricing MDX pages,
  archive-storage for trend pages).

### YMYL guardrails enforced

- Every price page-stamps the snapshot date in the rendered HTML.
- Built-in "verify directly with the operator before visiting" caveat
  on every menu block.
- No affiliate routing. No Leafly/Weedmaps/operator-site outbound
  links from price surfaces.
- Operator's own site always wins over directory cross-references.
- Failed extraction = unavailable, never invented data (verified
  for White Mountain Craft Cannabis; every category "unavailable").

### Built dist (post-build verification)

- `grep -c 'menu-block' dist/guides/founding-farmers-dispensary/index.html` → 2
- `grep -c 'Apple Fritter' dist/guides/founding-farmers-dispensary/index.html` → 1
- `grep -c 'Tourma Cannabis' dist/guides/founding-farmers-dispensary/index.html` → 1
- smoke-200: 277/277 routes 0 broken (production untouched, dist baseline check)
- 84 products now appear in the founding-farmers franchise page HTML.

### Deferred items (named explicitly, not shipped)

- **Other 16 franchise pages** (420 Mules Bar Harbor, High Road
  Gray, Hidden Greens Buxton, Bayside Bud Shack, Lakewood, Lifted
  Houlton, Just Baked Lincoln, Landrace Casco, Highbrow Cannabis,
  Botany Cannabis, Above All Greenery, Glass Cook, White Mountain
  Craft Cannabis [SPA-blocked], Eclipse, Great Atlantic Puffin
  Company, Healing Community MEDCo): each goes through one
  operator-extraction round in a future sprint.
- **Cross-operator strain heatmap** (`best-<strain>-in-maine.mdx`
  pages) requires ≥3 operators with snapshots. Current count: 2.
- **Cron wiring** (daily `menu-extract.cjs` run for each registry
  operator, with diff alerts) — out of scope this round.
- **Strain-synonym dictionary** (Apple Fritter = Apfelstrudel = AF)
  for cross-operator joining — designed but not implemented.
- **THC/CBD percentage capture** from per-product detail pages — the
  current category-pages-only extraction can't surface these; a
  follow-on extraction round fetches detail pages.
- **Real-time freshness pipeline** — explicitly out of scope; the
  snapshot-date stamp is the only signal until otherwise.

### Methodological discovery (logged in pitfall catalog)

- **Blaze-style SPA menu rendering is the dominant blocker.** The
  Blaze e-commerce platform (used by ffmaine.com) hides live prices
  behind JavaScript. Static-page extraction works for the *category
  archive* pages (which Blaze server-renders) but not the *product
  detail* pages. Of the 8 standard cannabis categories per operator,
  flower / edibles / concentrates / prerolls / drinks / glass are
  plaintext-extractable. Vapes + topicals are SPA-hidden
  everywhere the Blaze platform is in use. This shape likely holds
  for many Maine operators.
- **Bypass costs are real.** A headless-browser fallback (Playwright)
  would yield the missing categories but per the visual-design-survey
  skill, the 548-render 2026-07-13 audit degraded Steve's shared
  desktop. Lightweight per-category captures (one fetch per
  category, ~3-5s each) would be acceptable; a daily cron across
  many operators is the high-cost option. Decision deferred until
  ≥5 more operators are in scope.

### First measurable re-poke

After Vercel deployment (T+5-10 minutes), `curl
https://mainedispensaryguide.com/guides/founding-farmers-dispensary`
should show the rendered "84 products captured" caption. Coalesce
trend signals measurable in Q1 2027 once ≥3 operators are added.


## Dark-Spot Cleanup Round 2 (executed 2026-07-14, parent agent)

Branch `chore/content-health-dark-spot-cleanup-2026-07-14` pending verification.
Worktree: `.worktrees/dark-spot-cleanup-2026-07-14` from fetched
`origin/main`@`be74317e` (the parent agent's menu-data-pipeline ship).

### Trigger
Operator pattern 2026-07-14: "process and analyze the data we have
available and identify gaps in data that would be useful to track."
Plus the standing goal of polishing the whole site, not just the
franchise layer. Ran a fresh `content-health.cjs` pass against the
post-menu-pipeline worktree (the largest untouched surface area in the
MDG codebase) to surface what audit-driven fixes the next sprint
should pick up.

### Surfaced findings — classified and dispatched

The `node scripts/check/content-health.cjs` sweep reported 7 failures.
Each was investigated against the live code to confirm whether the
finding is a real bug or a false-positive driven by a check that
doesn't understand a known pattern. Classification:

**Real bugs (auto-fix this round):**

1. **`/embed/opt-in-tracker` meta description too long (225 chars).**
   Cap is ~160 per Google's SERP truncation rule. Trimmed prose by
   removing redundant clauses. New length: 111 chars.

2. **`/guides/maine-dispensary-costs` line 364 `href="/guides/"` under
   `trailingSlash: never`.** Trailing slash on directory anchor triggers
   a redirect (extra roundtrip + minor SEO hit). Changed to
   `href="/guides"` for parity with every other internal link in the
   file. Project-wide grep `href="/[a-z_-]+/"` returned no other
   offenders — this was the only trailing-slash trap on a top-level
   internal-link pattern.

**False positives (named explicitly, not shipped):**

3. **`/cite/[...rest]` orphan + `/cite/<slug>` ↔ `/cite/<slug>-hash>`
   duplicate meta description.** Both are by-design: the catch-all
   `[...rest].astro` route serves both the bare `/cite/<slug>` and the
   full DOI-style-hash `/cite/<slug>-<hash>` permalinks as defined in
   `apps/maine-cannabis/docs/seo/menu-data-pipeline.md` and the
   `citations.json` registry head comment (per the cite-this
   extraction). The "duplicate" meta description is the same citation
   rendered through two URL shapes — Google is told this via canonical
   `rel="canonical"` on each hash page. Visitors reach these via
   external journalism (PDF, citation, tweet) and via the citations
   registry, not via static `href` tags. Closing the orphan by
   adding an inbound link would mislead crawlers.

4. **`/embed/roi-calculator` orphan.** Same pattern as `/embed/opt-in-tracker`:
   embeddable iframe-ready widget for journalists and research sites,
   designed for inclusion via `<iframe src=...>`. Has no inbound
   `href` tags by design (canonical points at `/roi-calculator`).
   Google does not index it (`noindex, nofollow`).

### What this round did NOT touch

- Visual polish refactor owned by parallel session
  `design/refined-editorial-foundation-20260713` (3 commits ahead of
  origin/main, no integration yet, paths do not overlap with this
  worktree's changes).
- 17-title cohort measurement window (still open; no cohort pages
  were touched).
- Operator-name blog misroutes flagged in earlier Hub entries — those
  are operator-name query misroutes the prior sessions chose to
  defer until the backlink campaign advances. Same deferred.
- Other 16 franchise pages (no menu data yet except for White Mountain
  Craft Cannabis which marked everything unavailable).

### Verification

- `node scripts/check/content-health.cjs` post-fix (with explicit
  `CONTENT_HEALTH_DIST` + `CONTENT_HEALTH_ROOT` env vars, since the
  script's default paths resolve relative to its own location in the
  primary checkout, not a worktree): **2 of 7 surfaced failures auto-fixed
  this round** (the embed meta desc + the costs-page trailing slash); the
  remaining 5 reports break down as **4 design-intentional false
  positives** (cite-hash permalinks + embed iframe, documented in the
  Hub named-explicit section) **plus 1 known limitation**: the
  `trailing-slash internal links` source-level scan does not
  currently recurse into the worktree's source root automatically;
  `CONTENT_HEALTH_ROOT` must be set explicitly. Future work item, not
  a regression.
- Build status pending verify-iterate.
- The trailing-slash pattern was the only `href="/[a-z_-]+/"` offender
  in the repo (project-wide grep returned zero other matches), so no
  similar traps exist on other surfaces.
- Live confirmation: production HTML at
