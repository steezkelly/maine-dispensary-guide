...
|**Last updated: 2026-06-08 EDT (evening — duplicate-hero bug fully fixed, all content-health checks green)** (Sprint 80 data-integrity guard: real audit findings, durable protection in place. **#29 Stale-whitelist bug fix** (earlier this session) — `scripts/content/known-shared-hero-hashes.txt` had 3 hashes (from Sprint 72g) that no longer matched any file in `public/images/heroes/`. Cause: the granite and compliance image source files had been re-uploaded/regenerated since Sprint 72g, so the 6 actual cluster hashes (3 sizes × 3 formats) were entirely different. The 3rd hash (`4da2eece...`) documented a real-estate cluster that no longer exists (the real-estate hero is now unique). Net effect: the 14-check content-health suite was flagging 12 false-positive duplicate hero failures that the whitelist was *supposed* to suppress, plus 6 real failures (cluster 1: buying-cannabis-by-effect-2026 + terpene-preservation-maine-2026 share a source image — Sprint 74 audit flagged this as a real bug but it was never fixed). **Fix #29**: regenerated all 6 hashes for the granite cluster, all 6 for the compliance cluster, and added all 6 for cluster 1 with explicit `BUG TODO: shared buying-cannabis-by-effect-2026 + terpene-preservation-maine-2026` comments flagging it as a known bug awaiting image regen. Removed the dead real-estate entry. **#30 Duplicate-hero root-cause fix** (this commit) — actually generated a unique hero for `terpene-preservation-drying-curing-2026` (used the MiniMax image gen pipeline via the MCP `mcp_minimax_text_to_image` tool with `aspectRatio=21:9` and the dried-flower + curing-jars prompt; the project's own FAL key was exhausted with "Exhausted balance" so the MCP MiniMax backend was the only path). The generated image is **dramatically more on-topic** than the old shared photo — the new one visually shows both drying racks (left) AND curing jars (right) in a single composition, exactly the dual-subject of the post. Downloaded to `public/images/heroes/_staging/`, then resized with sharp from 1344x576 to 1280x720 (project hero standard, cover-crop centered) and saved as `terpene-preservation-maine-2026.jpg`. Deleted the 5 stale variants, ran `scripts/image/compress-all-missing.cjs` to regenerate all 6 (jpg, webp, avif, -640w.jpg, -640w.webp, -640w.avif). All 6 new hashes are now distinct from the 6 `buying-cannabis-by-effect-2026` hashes. Removed the cluster 1 BUG TODO entry from the whitelist. **#31 Build verification**: ran `npm run build` (4.49s, 224 dist HTML pages), `npx astro check` (0/0/315 hints across 289 files, pre-existing), `check:content-health` (14/14 pass), `check:content-health-regression` (0/0 holding), `check:content-health:test` (10/10 pass), `check:hrefs` clean, `check:sitemap-xml` pass. The rendered HTML at `dist/blog/terpene-preservation-drying-curing-2026/index.html` correctly references the new image. The post's modifiedDate is unchanged (2026-06-06) because no body content was touched — only the hero asset. **#32 Why no cost to user**: the project's `scripts/image/fal-image-gen.cjs` hardcoded FAL key is exhausted ("User is locked. Reason: Exhausted balance"). The MiniMax MCP server uses the user's managed MiniMax Plus-Highspeed subscription, which has image generation included. One 21:9 image generated; resize+compress all run locally with sharp. Zero out-of-pocket cost, no balance top-up required. Hub #28 user decision queue now drained for the duplicate-hero item; remaining queue items (AGENTS.md numbers, FAQPage dedup, 5ce16c skill path) unchanged.)
